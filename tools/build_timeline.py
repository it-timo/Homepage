...
def main():
  try:
    ...
    generated=[]
    allowed={'album','track','project','repository','article','note','project-journal','music-commentary','video','experiment','milestone'}
    for entity in graph['entities']:
      if entity['type'] not in allowed or not (entity.get('date') or entity.get('year')): continue
      # Album-level years do not turn every inherited track year into a separate
      # timeline event. A song enters the timeline only with its own exact date.
      if entity['type']=='track' and not entity.get('date'): continue
